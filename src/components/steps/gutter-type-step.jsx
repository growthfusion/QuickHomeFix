...
function GutterTypeStep() {
  ...
  const [selectedType, setSelectedType] = useState(formData.gutterType || null);
      ...
      <Card className="w-full max-w-sm bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
        ...
        <CardContent className="p-6 sm:p-8">
          <div className="text-center mb-6">
            <h2 className="text-lg sm:text-xl font-bold text-gray-900">What type of gutter service do you need?</h2>
          </div>
          <div className="border border-gray-200 rounded-xl overflow-hidden mb-6">
            {gutterTypes.map((type, idx) => (
              <button key={type.id} type="button" onClick={() => handleTypeSelect(type.id, type.questionText, type.answerText)}
                className={`w-full flex items-center gap-3 px-5 py-4 text-left transition-colors ${idx !== gutterTypes.length - 1 ? "border-b border-gray-200" : ""} ${selectedType === type.id ? "bg-blue-50" : "bg-white"}`}>
