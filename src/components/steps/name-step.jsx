...
function NameStep() {
  ...
  const [focusedField, setFocusedField] = useState(null); // <-- FIX: removed type

  const isFormValid = formData.firstName && formData.lastName;

  // GTM question labels (centralized)
  const Q_FIRST = "Enter Your First Name";
  const Q_LAST  = "Enter Your Last Name";

  // GTM push helper (JS-only)
  const pushFormEvent = (question, answer) => {
    if (typeof window === "undefined") return; // <-- FIX: SSR-safe
  ...
  // FIX: removed "(e?: React.FormEvent)"
  const handleSubmit = (e) => {
    if (e) e.preventDefault();
    if (!isFormValid) return;

    pushFormEvent(Q_FIRST, formData.firstName || "");
    pushFormEvent(Q_LAST,  formData.lastName  || "");
