...
function CompleteStep() {
  ...
  // This should be determined by your app state
  const isNewQuoteRequest = true;
  ...
  const getServiceCards = () => {
    ...
    const serviceDetails = {
      "roofing": { name: "Roofing", image: "/roofing_services.jpg", description: "Premium roofing solutions for any home", benefit: "50-year warranty available" },
      "solar": { name: "Solar Energy", image: "/Solar.jpg", description: "Save on energy costs with clean solar power", benefit: "Up to 30% energy savings" },
      "windows": { name: "Windows", image: "/window_services.jpg", description: "Energy-efficient windows for comfort and savings", benefit: "Reduces energy costs by 15%" },
      "gutter": { name: "Gutters", image: "/gutter_services.jpg", description: "Quality gutter systems to protect your home", benefit: "Prevents foundation damage" },
      "bath": { name: "Bath Remodeling", image: "/walkin_tub_services.png", description: "Modern bathroom renovations for enhanced comfort", benefit: "Increases home value" },
      "shower": { name: "Walk-in Shower", image: "/walkin_shower_services.png", description: "Modern, accessible shower solutions", benefit: "Quick 1-day installation" },
      "tub": { name: "Walk-in Tub", image: "/walkin_tub_services.png", description: "Safe, comfortable bathing experience", benefit: "Enhanced safety features" }
    };
    
    // Define specific service sets for each selected service
    const serviceSets = {
    ...
    // Get the appropriate set for the selected service (default to roofing)
    const selectedService = formData.service || "roofing";
    const serviceSet = serviceSets[selectedService] || serviceSets["roofing"];
    
    // Map the service IDs to the full details
    return serviceSet.map(id => ({
      id,
      ...serviceDetails[id]
    }));
  };
  ...
  const handleServiceSelect = () => {
    if (isNewQuoteRequest) {
      // Redirect to /roofing for all services
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-5 mb-12">
          {serviceCards.map((service, index) => (
            <div 
              ...
              onClick={isNewQuoteRequest ? handleServiceSelect : undefined}
            >
              ...
              <div className="p-5 flex flex-col flex-grow bg-gradient-to-b from-white to-gray-50">
                <h4 className="font-bold text-lg mb-2 text-gray-800 group-hover:text-green-600 transition-colors">{service.name}</h4>
