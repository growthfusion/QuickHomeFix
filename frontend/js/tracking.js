...
/* ═══════════════════════════════════════════
   ...
   ═══════════════════════════════════════════ */
var API_BASE =
  (typeof QHF_CONFIG !== "undefined" && QHF_CONFIG.API_BASE_URL) || "";
var API_TIMEOUT_MS =
  (typeof QHF_CONFIG !== "undefined" && QHF_CONFIG.API_TIMEOUT_MS) || 90000;
...
function trackVirtualPageview(service, stepName, stepNumber, totalSteps) {
  ...
  window.dataLayer.push({
    ...
    virtualPageTitle:
      (stepLabels[stepName] || stepName) + " | " + service + " | QuickHomeFix",
    stepName: stepName,
    stepNumber: stepNumber,
    totalSteps: totalSteps,
    service: service,
    stepLabel: stepLabels[stepName] || stepName,
  });
}
...
/* ═══════════════════════════════════════════
   4. REDTRACK – Click Beacon only
   (Postback removed — handled by Cloud Run)
   ═══════════════════════════════════════════ */
...
var LEADPOST_PARTNER_SOURCE_ID =
  (typeof QHF_CONFIG !== "undefined" &&
    QHF_CONFIG.LEADPOST_PARTNER_SOURCE_ID) ||
  "CampaignA";
var LEADPOST_PUBLISHER_SUB_ID =
  (typeof QHF_CONFIG !== "undefined" && QHF_CONFIG.LEADPOST_PUBLISHER_SUB_ID) ||
  "123456";
...
function getLeadpostAttribution(formData) {
  ...
  return {
    partnerSourceId:
      (formData.partnerSourceId || "").trim() || LEADPOST_PARTNER_SOURCE_ID,
    publisherSubId:
      (formData.publisherSubId || "").trim() || LEADPOST_PUBLISHER_SUB_ID,
  };
}
  ...
  return placesAutocomplete(zipcode)
    ...
    .then(function (detail) {
      if (detail.city && detail.state)
        return { city: detail.city, state: detail.state.toUpperCase() };
...
function buildThumbTackUtmData() {
  ...
  var utmCampaign =
    params.get("utm_campaign") || params.get("rt_cid") || getSourceId() || "";
  var utmContent =
    params.get("utm_content") || params.get("utm_rt_ad") || getRtAd() || "";
  var utmSubid = params.get("utm_subid") || getClickId() || "";
  var utmUserHash =
    params.get("utm_user_hash") ||
    params.get("utm_facebook_click_id") ||
    getFbclid() ||
    "";
...
/** POST /api/thumbtack/businesses — body: { searchQuery, zipCode, utmData } */
